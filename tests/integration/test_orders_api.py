...
class TestOrders:
    def _setup_panier(self, client, user_id, stock=10, price=100.0, quantity=2):
        p = client.post('/products/', json={'name': 'Produit Commande', 'price': price, 'stock': stock}).json()
        client.post(f'/cart/?user_id={user_id}', json={'product_id': p['id'], 'quantity': quantity})
        return p
    ...
    def test_commande_avec_coupon(self, client, api_coupon):
        ...
        order = client.post('/orders/', json={'user_id': 204, 'coupon_code': api_coupon['code']}).json()
    ...
    def test_statut_pending_vers_confirmed(self, client):
        ...
        response = client.patch(f'/orders/{order["id"]}/status', json={'status': 'confirmed'})
    ...
    def test_coupon_inexistant_retourne_404(self, client):
        ...
        response = client.post('/orders/', json={'user_id': 207, 'coupon_code': 'FAKECODE'})
        assert response.status_code == 404

    def test_get_commande_par_id(self, client):
        """Test Q3.4"""
        self._setup_panier(client, user_id=208)
        created_order = client.post('/orders/', json={'user_id': 208}).json()
        ...
        assert data['total_ttc'] == created_order['total_ttc']