...
def reserver_stock(product: Product, quantite: int, session: Session) -> Product:
    if not verifier_stock(product, quantite):
        raise ValueError(f"Stock insuffisant pour {product.name}: {product.stock} disponible(s), {quantite} demandé(s).")
    product.stock -= quantite
    session.commit()
    session.refresh(product)
    cache_key = f"product:{product.id}:stock"
    redis_client.delete(cache_key)
    logger.info(f"Stock reserve: {product.id}, qty: {quantite}")
    return product